...
test('inspection, finalization, marker and standard Quit menu persist', async () => {
  ...
  try {
    ...
    await expect(page.getByRole('heading', { name: 'Centering Front', exact: true })).toBeVisible()
    await expect(page.getByRole('heading', { name: 'Centering Back', exact: true })).toBeVisible()
    await expect(page.getByLabel('Centering', { exact: true })).toHaveCount(1)
    const positionLabels = ['Top Left', 'Top Right', 'Bottom Left', 'Bottom Right', 'Upper Left', 'Upper Right', 'Lower Left', 'Lower Right']
    for (const face of ['Front', 'Back']) for (const [index, label] of positionLabels.entries()) await page.getByLabel(`${face} ${label}`, { exact: true }).fill(((face === 'Front' ? 200 : 300) + index + 1) / 100 + '')
    await page.getByRole('button', { name: 'Overview', exact: true }).click()
    await page.getByRole('button', { name: 'Inspection', exact: true }).click()
    for (const face of ['Front', 'Back']) for (const [index, label] of positionLabels.entries()) {
      await expect(page.getByLabel(`${face} ${label}`, { exact: true })).toHaveValue((((face === 'Front' ? 200 : 300) + index + 1) / 100).toFixed(2))
      await page.getByLabel(`${face} ${label}`, { exact: true }).fill(face === 'Front' ? '2.5' : '2')
    }
    await expect(page.locator('svg.measurement-helper')).toHaveCount(16)
    for (const position of ['verticalLeftTop', 'verticalLeftBottom', 'verticalRightTop', 'verticalRightBottom', 'horizontalUpperLeft', 'horizontalUpperRight', 'horizontalLowerLeft', 'horizontalLowerRight']) await expect(page.locator(`svg[data-position="${position}"]`)).toHaveCount(2)
    expect(await page.locator('svg.measurement-helper').evaluateAll(icons => icons.every(icon => icon.getAttribute('aria-hidden') === 'true' && !icon.hasAttribute('tabindex')))).toBe(true)
    const measurementLabels = ['Front', 'Back'].flatMap(face => positionLabels.map(label => `${face} ${label}`))
    async function checkMeasurementFocusOrder(): Promise<void> {
      expect(await page.locator('.centering-face .measurement-inputs input').evaluateAll(inputs => inputs.map(input => input.getAttribute('aria-label')))).toEqual(measurementLabels)
      await page.getByLabel(measurementLabels[0], { exact: true }).focus()
      for (const label of measurementLabels.slice(1)) {
        await page.keyboard.press('Tab')
        await expect(page.getByLabel(label, { exact: true })).toBeFocused()
      }
      for (const label of measurementLabels.slice(0, -1).reverse()) {
        await page.keyboard.press('Shift+Tab')
        await expect(page.getByLabel(label, { exact: true })).toBeFocused()
      }
    }
    await checkMeasurementFocusOrder()
    for (const face of ['Front', 'Back']) for (const [index, label] of positionLabels.entries()) expect(await page.getByLabel(`${face} ${label}`, { exact: true }).evaluate(input => input.closest('label')?.querySelector('svg')?.getAttribute('data-position'))).toBe(['verticalLeftTop', 'verticalRightTop', 'verticalLeftBottom', 'verticalRightBottom', 'horizontalUpperLeft', 'horizontalUpperRight', 'horizontalLowerLeft', 'horizontalLowerRight'][index])
    await page.getByLabel('Back Top Left', { exact: true }).blur()
    await expect(page.getByLabel('Back Top Left', { exact: true })).toHaveValue('2.00')
    for (const [face, values] of [['Front', ['1.38', '2.62', '2.44', '2.44']], ['Back', ['2.62', '1.38', '1.56', '1.56']]] as const) {
      for (const [index, label] of ['Bottom Left', 'Bottom Right', 'Upper Right', 'Lower Left'].entries()) await page.getByLabel(`${face} ${label}`, { exact: true }).fill(values[index])
    }
    for (const [index, ratios] of [['64.4 / 35.6', '48.8 / 51.2', '50.6 / 49.4', '49.4 / 50.6'], ['43.3 / 56.7', '59.2 / 40.8', '56.2 / 43.8', '43.8 / 56.2']].entries()) {
      const face = page.locator('.centering-face').nth(index)
      for (const [ratioIndex, label] of ['Vertical Left · Top / Bottom', 'Vertical Right · Top / Bottom', 'Horizontal Upper · Left / Right', 'Horizontal Lower · Left / Right'].entries()) await expect(face.locator('.ratio').filter({ hasText: label }).locator('strong')).toHaveText(ratios[ratioIndex])
    }
    ...
    await page.getByLabel('Centering notes text').fill('Judged from both faces')
    ...
    await page.getByLabel('Back Top Left', { exact: true }).fill('2.35')
    await page.getByRole('button', { name: 'Overview', exact: true }).click()
    await page.getByRole('button', { name: 'Inspection', exact: true }).click()
    await expect(page.getByLabel('Back Top Left', { exact: true })).toHaveValue('2.35')
    ...
    for (const face of await page.locator('.centering-face').all()) {
      const groups = await face.locator('.measurement-pair').evaluateAll(panels => panels.map(panel => ({ labels: Array.from(panel.querySelectorAll('input')).map(input => input.getAttribute('aria-label')), top: panel.getBoundingClientRect().top, left: panel.getBoundingClientRect().left })))
      expect(groups.map(group => group.labels.map(label => label!.replace(/^(Front|Back) /, '')))).toEqual([['Top Left', 'Top Right'], ['Bottom Left', 'Bottom Right'], ['Upper Left', 'Upper Right'], ['Lower Left', 'Lower Right']])
      expect(groups[0].top).toBe(groups[2].top); expect(groups[1].top).toBe(groups[3].top)
      expect(groups[0].left).toBe(groups[1].left); expect(groups[2].left).toBe(groups[3].left)
      expect(groups[1].top).toBeGreaterThan(groups[0].top); expect(groups[2].left).toBeGreaterThan(groups[0].left)
    }
    await page.locator('.centering-face').first().scrollIntoViewIfNeeded()
    await page.screenshot({ path: 'test-results/centering-front-laptop.png' })
    await page.locator('.centering-face').last().scrollIntoViewIfNeeded()
    await page.screenshot({ path: 'test-results/centering-back-laptop.png' })
    await application.evaluate(({ BrowserWindow }) => { const window = BrowserWindow.getAllWindows()[0]; window.setMinimumSize(640, 600); window.setSize(720, 800) })
    await checkMeasurementFocusOrder()
    for (const face of await page.locator('.centering-face').all()) {
      const groups = await face.locator('.measurement-pair').evaluateAll(panels => panels.map(panel => ({ top: panel.getBoundingClientRect().top, left: panel.getBoundingClientRect().left })))
      for (let index = 1; index < groups.length; index++) { expect(groups[index].left).toBe(groups[0].left); expect(groups[index].top).toBeGreaterThan(groups[index - 1].top) }
    }
    await page.locator('.centering-face').last().scrollIntoViewIfNeeded()
    await page.screenshot({ path: 'test-results/centering-back-narrow.png' })
    const facesLayout = await page.locator('.centering-face').evaluateAll(elements => elements.map(element => ({ top: element.getBoundingClientRect().top, width: element.getBoundingClientRect().width, scrollWidth: element.scrollWidth })))
    expect(facesLayout[1].top).toBeGreaterThan(facesLayout[0].top)
    for (const face of facesLayout) expect(face.scrollWidth).toBeLessThanOrEqual(Math.ceil(face.width))
    ...
    await page.getByRole('button', { name: 'Inspection', exact: true }).click()
    await page.getByLabel('Back Bottom Right', { exact: true }).fill('1.80')
    ...
    await expect(page.getByLabel('Back Bottom Right', { exact: true })).toHaveValue('1.80')
    await expect(page.getByText('Finalized — changes pending', { exact: true }).first()).toBeVisible()
    await page.getByRole('button', { name: 'Finalize again', exact: true }).click()
    await expect(page.getByText('Finalized', { exact: true }).first()).toBeVisible()
    await page.getByLabel('Centering notes', { exact: true }).click()
    await expect(page.getByLabel('Centering notes text')).toHaveValue('Judged from both faces')
    await page.getByRole('button', { name: /Marker 1, front/ }).click()
    await expect(page.getByLabel('Marker 1 note')).toHaveValue('Tiny surface line')
    await page.getByRole('button', { name: 'Photos', exact: true }).click()
    await expect(page.getByTitle('replacement.png')).toBeVisible()
  ...
  try {
    ...
    await page.evaluate(async () => {
      for (let index = 1; index <= 6; index += 1) {
        ...
        for (const key of ['frontVerticalLeftTop', 'frontVerticalLeftBottom', 'frontVerticalRightTop', 'frontVerticalRightBottom', 'frontHorizontalUpperLeft', 'frontHorizontalUpperRight', 'frontHorizontalLowerLeft', 'frontHorizontalLowerRight', 'backVerticalLeftTop', 'backVerticalLeftBottom', 'backVerticalRightTop', 'backVerticalRightBottom', 'backHorizontalUpperLeft', 'backHorizontalUpperRight', 'backHorizontalLowerLeft', 'backHorizontalLowerRight'] as const) inspection[key] = 200
        if (index === 1) Object.assign(inspection, {
          frontVerticalLeftBottom: 138, frontVerticalRightBottom: 262, frontHorizontalUpperRight: 244, frontHorizontalLowerLeft: 244,
          backVerticalLeftBottom: 262, backVerticalRightBottom: 138, backHorizontalUpperRight: 156, backHorizontalLowerLeft: 156
        })
    ...
    await page.evaluate(async () => {
      ...
      for (const slot of ['corner_top_left', 'edge_top', null] as const) {
        const photo = await window.cards.choosePhotos(card.id, slot)
        if (!photo.ok) throw new Error(photo.error)
        if (!photo.value) throw new Error('Report detail photo import was cancelled')
        if (slot === null) {
          const back = await window.cards.addMarker(card.id, 'back', 0.6, 0.85)
          if (!back.ok) throw new Error(back.error)
          const note = await window.cards.saveMarker(back.value.id, 'Back edge nick')
          if (!note.ok) throw new Error(note.error)
          const title = await window.cards.savePhotoTitle(photo.value.photos.find(item => item.slot === null)!.id, 'Raking light evidence')
          if (!title.ok) throw new Error(title.error)
          const evidence = await window.cards.setPhotoMarkers(title.value.id, [marker.value.id, back.value.id])
    ...
    await expect(page.getByText('Publishes new and changed cards from this library and updates the combined catalogue.')).toBeVisible()
    ...
    await page.getByRole('button', { name: 'Rebuild Public Site…', exact: true }).click()
    await expect(page.getByRole('dialog', { name: 'Rebuild the public site?' })).toBeVisible()
    ...
    await expect(publicPage.getByRole('heading', { name: 'Centering Front', exact: true })).toBeVisible()
    await expect(publicPage.getByRole('heading', { name: 'Centering Back', exact: true })).toBeVisible()
    await expect(publicPage.locator('.public-centering-face').first()).toContainText('~0.4° CW')
    await expect(publicPage.locator('.public-centering-face').last()).toContainText('~0.4° CCW')
    expect(await publicPage.locator('.centering-panel,.photo-report-grid .photo-section,.defects-panel').evaluateAll(sections => sections.map(section => section.querySelector('h2')?.textContent))).toEqual(['Centering Front', 'Corners', 'Edges', 'Defect Map'])
    await expect(publicPage.getByRole('heading', { name: 'Defect Map', exact: true })).toHaveCount(1)
    await publicPage.getByRole('button', { name: 'Select defect 2, back', exact: true }).click()
    await expect(publicPage.locator('.defect-detail.active')).toContainText('Back edge nick')
    await publicPage.locator('.defect-detail.active .evidence-photo').getByRole('button').click()
    await expect(publicPage.getByRole('dialog')).toBeVisible()
    await publicPage.keyboard.press('Escape')
    await expect(publicPage.getByRole('dialog')).not.toBeVisible()
    await publicPage.screenshot({ path: 'test-results/public-front-back-desktop.png', fullPage: true })
    await application.evaluate(({ BrowserWindow }) => BrowserWindow.getAllWindows().find(window => window.webContents.getURL().startsWith('http://'))?.setSize(1000, 760))
    await publicPage.screenshot({ path: 'test-results/public-front-back-laptop.png', fullPage: true })
    await publicPage.locator('.centering-panel').screenshot({ path: 'test-results/public-centering-laptop.png' })
    expect(await publicPage.evaluate(() => document.documentElement.scrollWidth <= window.innerWidth)).toBe(true)
    await application.evaluate(({ BrowserWindow }) => BrowserWindow.getAllWindows().find(window => window.webContents.getURL().startsWith('http://'))?.setSize(420, 800))
    await publicPage.screenshot({ path: 'test-results/public-front-back-mobile.png', fullPage: true })
    await publicPage.locator('.centering-panel').screenshot({ path: 'test-results/public-centering-mobile.png' })
    expect(await publicPage.evaluate(() => document.documentElement.scrollWidth <= window.innerWidth)).toBe(true)
    await application.evaluate(({ BrowserWindow }) => BrowserWindow.getAllWindows().find(window => window.webContents.getURL().startsWith('http://'))?.setSize(1280, 900))
    const markerInsideMap = await publicPage.locator('.defect-marker').evaluateAll(markers => markers.every(marker => {
    ...
    expect(await publicPage.locator('.defects-panel').evaluate(section => section.getBoundingClientRect().top >= document.querySelector('.photo-report-grid')!.getBoundingClientRect().bottom)).toBe(true)
    await publicPage.locator('.photo-report-grid').screenshot({ path: 'test-results/public-corner-edge-sections.png' })
    await publicPage.locator('.defects-panel').screenshot({ path: 'test-results/public-defect-section.png' })
    ...
    if (!publicAddress || typeof publicAddress === 'string') throw new Error('Static validation server unavailable')
    await publicPage.goto(`http://127.0.0.1:${publicAddress.port}/cards/0000000002/`)
    await expect(publicPage.getByText('No defects documented.', { exact: true })).toBeVisible()
    expect(await publicPage.locator('.photo-section,.defects-panel').evaluateAll(sections => sections.map(section => section.querySelector('h2')?.textContent))).toEqual(['Corners', 'Edges', 'Defect Map'])
    await publicPage.screenshot({ path: 'test-results/public-report-no-defects.png', fullPage: true })
    await publicPage.locator('.defects-panel').screenshot({ path: 'test-results/public-no-defects-section.png' })
    await publicPage.goto(`http://127.0.0.1:${publicAddress.port}/project/cards/0000000001/`)
    await expect(publicPage.getByRole('heading', { name: 'Defect Map', exact: true })).toBeVisible()
    await publicPage.getByRole('button', { name: 'Select defect 2, back', exact: true }).click()
    await expect(publicPage.locator('.defect-detail.active')).toContainText('Back edge nick')
    await publicPage.locator('.defect-detail.active .evidence-photo').getByRole('button').click()
    await expect(publicPage.getByRole('dialog')).toBeVisible()
